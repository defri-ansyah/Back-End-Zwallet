...
const transactions = {
  transfer: (receiverId, senderId, amount, notes) => new Promise((resolve, reject) => {
  ...
  history: (id, offset, perPage) => new Promise((resolve, reject) => {
  ...
  historyCount: (id) => new Promise((resolve, reject) => {
    connection.query("SELECT history.* FROM (SELECT st.*, receiver.username AS name, receiver.phone_number AS phone, 'expense' AS type FROM `transactions` AS st JOIN users AS receiver ON receiver.id = st.receiver_id WHERE st.sender_id = ? UNION ALL SELECT rt.*, sender.username AS name, sender.phone_number AS phone , 'income' as type FROM `transactions` AS rt  JOIN users AS sender ON sender.id = rt.sender_id WHERE rt.receiver_id = ? ) as history ORDER BY created_at ASC", [id, id], (error, results) => {
      if (!error) {
        resolve(results);
      } else {
        reject(error);
